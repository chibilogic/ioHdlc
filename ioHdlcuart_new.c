...
/**
 * @file    ioHdlcuart_new.c
 * @brief   OS-agnostic UART adapter for frame-oriented HDLC drivers.
 */
...
bool ioHdlcUartNew_init(ioHdlcUartNew             *d,
                        ...
                        void                      *upper_ctx) {
  if (!d || !port || !port->ops || !params || !cfg || !pool ||
      !cfg->deliver_rx_frame)
    return false;
...
bool ioHdlcUartNew_start(ioHdlcUartNew *d) {
  ...
  d->port.ops->start(d->port.ctx, &d->params, &d->hal_cbs);
 ...
 */
static void s_arm_idle(ioHdlcUartNew *d) {
  (void)d->port.ops->rx_submit(d->port.ctx, &d->rx_flagoctet, 1);
}

static void s_arm_inframe_from(ioHdlcUartNew *d, size_t offset, size_t need) {
 ...
 */
static void s_deliver_and_continue(ioHdlcUartNew *d) {
 ...
 */
static void s_handle_rx_error(ioHdlcUartNew *d) {
 ...
 */
static void s_hal_on_rx(void *cb_ctx, bool timed_out) {
  ioHdlcUartNew *d = (ioHdlcUartNew *)cb_ctx;
  if (!d || !d->started) return;
...
static void s_hal_on_tx_done(void *cb_ctx, void *cookie) {
  (void)cb_ctx;
  (void)cookie;
  /* Upper layer can manage TX completion by passing 'cookie' to its own  */
  /* bookkeeping. This adapter does not hold TX queues itself.             */
}
