 ...
 */
/**
 * @file    test_checkpoint_twa.c
 * @brief   Test checkpoint retransmission in TWA mode.
 *
 * @details Validates:
 *          - Scenario A.1: Frame loss with window full (TWA without REJ)
 *          - Checkpoint retransmission behavior (ISO 13239 5.6.2.1)
 *          - P/F bit protocol correctness
 *          - Frame recovery after checkpoint failure
 *
 * @note    TWA = Two-Way Alternate: Primary sends P at end of window.
...
/*===========================================================================*/
/* Test: Scenario A.1.1 - Frame Loss with Window Full (TWA without REJ)    */
/*===========================================================================*/
...
bool test_A1_1_frame_loss_window_full_twa(const test_adapter_t *adapter) {
  ...
  test_printf("\r\n=== Test A.1.1: Frame Loss + Window Full (TWA, no REJ) ===\r\n");
  ...
  while (total_received < 96 && read_attempts < max_attempts) {
    ...
    if (received > 0) {
      test_printf("  Read attempt %d: +%d bytes (total now: %d)\r\n", 
                  read_attempts + 1, (int32_t)received, (int32_t)(total_received + received));
      total_received += (size_t)received;
    } else if (received == 0) {
      test_printf("  Read attempt %d: no data (timeout), total: %d bytes\r\n",
                  read_attempts + 1, (int32_t)total_received);
    } else {
  ...
  test_printf("\r\nTotal bytes received: %d (expected %d) after %d read attempts\r\n", 
              (int32_t)total_received, (int32_t)expected_size, read_attempts);
  
  if (total_received == expected_size) {
    test_printf("✅ All frames eventually delivered after checkpoint retransmission!\r\n");
  } else {
    test_printf("❌ Received %d bytes (expected %d) - retransmission failed!\r\n", 
                (int32_t)total_received, (int32_t)expected_size);
  }
